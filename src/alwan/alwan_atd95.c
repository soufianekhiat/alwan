...
static void retinal_xyz_to_lms(
    ...
    *L = ATD95_L_SCALE * ALWAN_POW(ALWAN_ABS(L_linear), ATD95_L_EXP) + ATD95_L_OFFSET;
    if (L_linear < ALWAN_LITERAL(0.0)) *L = -(*L);

    /* M cone response */
    alwan_scalar M_linear = ATD95_LMS_M_X * X_r + ATD95_LMS_M_Y * Y_r + ATD95_LMS_M_Z * Z_r;
    *M = ALWAN_POW(ALWAN_ABS(M_linear), ATD95_M_EXP) + ATD95_M_OFFSET;
    if (M_linear < ALWAN_LITERAL(0.0)) *M = -(*M);

    /* S cone response */
    alwan_scalar S_linear = ATD95_LMS_S_Y * Y_r + ATD95_LMS_S_Z * Z_r;
    *S = ATD95_S_SCALE * ALWAN_POW(ALWAN_ABS(S_linear), ATD95_S_EXP) + ATD95_S_OFFSET;
...
static alwan_scalar final_response(alwan_scalar value) {
    return value / (ATD95_FINAL_NORM + ALWAN_ABS(value));
}
...
int alwan_atd95_forward(
    ...
    if (ALWAN_ABS(A_2) > ALWAN_LITERAL(1e-10)) {
