...
int alwan_xyz_to_lab_map(alwan_scalar *lab_out,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        alwan_lab lab = alwan_xyz_to_lab_v(xyz, *white_xyz);
...
int alwan_lab_to_xyz_map(alwan_scalar *xyz_out,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        alwan_xyz xyz = alwan_lab_to_xyz_v(lab, *white_xyz);
...
int alwan_xyz_to_luv_map(alwan_scalar *luv_out,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        alwan_xyz xyz = {in_ptr[0], in_ptr[1], in_ptr[2]};
        alwan_luv luv = alwan_xyz_to_luv_v(xyz, *white_xyz);
...
int alwan_luv_to_xyz_map(alwan_scalar *xyz_out,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        alwan_xyz xyz = alwan_luv_to_xyz_v(luv, *white_xyz);
        out_ptr[0] = xyz.x; out_ptr[1] = xyz.y; out_ptr[2] = xyz.z;
    }
#endif
    return ALWAN_OK;
