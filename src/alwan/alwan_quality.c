...
alwan_scalar alwan_cct_kang_xy(alwan_vec2 const *xy) {
    ...
    alwan_scalar const h = ALWAN_LITERAL(1.0);  /* Step for numerical derivative */

    for (int iter = 0; iter < 20; iter++) {
        alwan_vec2 xy_calc;
        alwan_cct_to_xy_kang(&xy_calc, cct);

        /* Distance squared to target */
        alwan_scalar dx = xy_calc.v[0] - xy->v[0];
        alwan_scalar dy = xy_calc.v[1] - xy->v[1];
        alwan_scalar dist2 = dx * dx + dy * dy;

        /* Check convergence */
        if (dist2 < ALWAN_LITERAL(1e-16)) break;

        /* Compute gradient numerically */
        alwan_vec2 xy_plus, xy_minus;
        alwan_cct_to_xy_kang(&xy_plus, cct + h);
        alwan_cct_to_xy_kang(&xy_minus, cct - h);

        alwan_scalar dx_plus = xy_plus.v[0] - xy->v[0];
        alwan_scalar dy_plus = xy_plus.v[1] - xy->v[1];
        alwan_scalar dist2_plus = dx_plus * dx_plus + dy_plus * dy_plus;

        alwan_scalar dx_minus = xy_minus.v[0] - xy->v[0];
        alwan_scalar dy_minus = xy_minus.v[1] - xy->v[1];
        alwan_scalar dist2_minus = dx_minus * dx_minus + dy_minus * dy_minus;

        /* Gradient of distance squared */
        alwan_scalar grad = (dist2_plus - dist2_minus) / (ALWAN_LITERAL(2.0) * h);

        /* Second derivative (Hessian) */
        alwan_scalar hess = (dist2_plus - ALWAN_LITERAL(2.0) * dist2 + dist2_minus) / (h * h);

        if (ALWAN_ABS(hess) < ALWAN_LITERAL(1e-20)) break;

        /* Newton step */
        alwan_scalar delta = -grad / hess;

        /* Damping for stability */
        if (ALWAN_ABS(delta) > ALWAN_LITERAL(500.0)) {
            delta = (delta > 0) ? ALWAN_LITERAL(500.0) : ALWAN_LITERAL(-500.0);
        }
