...
ALWAN_INLINE alwan_scalar hunt_f_n_v(alwan_scalar x) {
    alwan_scalar const xp = ALWAN_POW(alwan_max(x, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.73));
...
ALWAN_INLINE alwan_scalar hunt_eccentricity_v(alwan_scalar hue) {
    alwan_scalar const h_s0 = ALWAN_LITERAL(20.14), h_s1 = ALWAN_LITERAL(90.0), h_s2 = ALWAN_LITERAL(164.25), h_s3 = ALWAN_LITERAL(237.53);
    alwan_scalar const e_s0 = ALWAN_LITERAL(0.8),   e_s1 = ALWAN_LITERAL(0.7),  e_s2 = ALWAN_LITERAL(1.0),    e_s3 = ALWAN_LITERAL(1.2);
    alwan_scalar interp;
    if (hue <= h_s1) {
...
ALWAN_INLINE alwan_vec3 hunt_adapt_v(
    ...
    if (discount) {
        ...
        alwan_scalar const La_p = ALWAN_POW(alwan_max(La, ALWAN_LITERAL(0.0)), ALWAN_ONE / ALWAN_LITERAL(3.0));
        for (i = 0; i < 3; i++) {
    ...
    if (helson_judd) {
        alwan_scalar const Y_b_Y_w = Y_b / ALWAN_SELECT(ALWAN_ABS(Y_w) > ALWAN_LITERAL(1e-12), Y_w, ALWAN_ONE);
        alwan_scalar const ref = hunt_f_n_v(Y_b_Y_w * FL * F_rgb[1]);
        for (i = 0; i < 3; i++) {
    ...
    if (p != ALWAN_LITERAL(0.0)) {
        alwan_scalar const p_rgb_sum = rgb_p.v[0] + rgb_p.v[1] + rgb_p.v[2];
        alwan_scalar const p_sum_safe = ALWAN_SELECT(ALWAN_ABS(p_rgb_sum) > ALWAN_LITERAL(1e-12), p_rgb_sum, ALWAN_ONE);
        for (i = 0; i < 3; i++) {
            alwan_scalar const p_rgb = ALWAN_LITERAL(3.0) * rgb_p.v[i] / p_sum_safe;
...
ALWAN_INLINE alwan_hunt_v_correlates alwan_hunt_forward_v(alwan_xyz xyz, alwan_hunt_viewing_conditions vc) {
    ...
    alwan_scalar const Y_w = vc.xyz_w.y;
    alwan_scalar const Y_b_in = (vc.xyz_b.y > ALWAN_LITERAL(0.0)) ? vc.xyz_b.y : vc.Yb;
    alwan_scalar const Y_b = (Y_b_in > ALWAN_LITERAL(0.0)) ? Y_b_in : ALWAN_LITERAL(20.0);
    alwan_scalar const La = vc.La;
    alwan_scalar const N_c = hunt_surround_Nc_v(vc.surround);
    alwan_scalar const N_b = hunt_surround_Nb_v(vc.surround);

    /* Induction factors: Hunt's approximation when the caller leaves them 0. */
    alwan_scalar const Yw_safe = ALWAN_SELECT(ALWAN_ABS(Y_w) > ALWAN_LITERAL(1e-12), Y_w, ALWAN_ONE);
    alwan_scalar const ind = ALWAN_LITERAL(0.725) * ALWAN_POW(alwan_max(Yw_safe / Y_b, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.2));
    alwan_scalar const N_cb = (vc.N_cb > ALWAN_LITERAL(0.0)) ? vc.N_cb : ind;
    alwan_scalar const N_bb = (vc.N_bb > ALWAN_LITERAL(0.0)) ? vc.N_bb : ind;

    /* Scotopic luminance of the illuminant, from CCT when not given. */
    alwan_scalar const cct = (vc.CCT_w > ALWAN_LITERAL(0.0)) ? vc.CCT_w : ALWAN_LITERAL(5000.0);
    alwan_scalar const L_AS = (vc.L_AS > ALWAN_LITERAL(0.0))
        ? vc.L_AS
    ...
    alwan_scalar const S_p   = (vc.S   > ALWAN_LITERAL(0.0)) ? vc.S   : xyz.y;
    alwan_scalar const S_w_p = (vc.S_w > ALWAN_LITERAL(0.0)) ? vc.S_w : Y_w;
    ...
    alwan_scalar const k = ALWAN_ONE / (ALWAN_LITERAL(5.0) * La + ALWAN_ONE);
    alwan_scalar const k4 = k * k * k * k;
    alwan_scalar const FL = ALWAN_LITERAL(0.2) * k4 * (ALWAN_LITERAL(5.0) * La) +
    ...
    alwan_vec3 const rgb   = alwan_mat3_mulv_v(HUNT_V_M_HPE, xyz_v);
    alwan_vec3 const rgb_w = alwan_mat3_mulv_v(HUNT_V_M_HPE, xyz_w_v);
    alwan_vec3 const rgb_p = alwan_mat3_mulv_v(HUNT_V_M_HPE, xyz_p_v);

    {
    alwan_vec3 const rgb_a  = hunt_adapt_v(rgb,   rgb_w, Y_w, Y_b, La, FL, rgb_p, vc.p,
                                          vc.helson_judd_effect, vc.discount_illuminant);
    alwan_vec3 const rgb_aw = hunt_adapt_v(rgb_w, rgb_w, Y_w, Y_b, La, FL, rgb_p, vc.p,
                                          vc.helson_judd_effect, vc.discount_illuminant);

    /* Achromatic post-adaptation signals. */
    alwan_scalar const A_a  = ALWAN_LITERAL(2.0) * rgb_a.v[0]  + rgb_a.v[1]  + rgb_a.v[2]  / ALWAN_LITERAL(20.0) - ALWAN_LITERAL(3.05) + ALWAN_ONE;
    alwan_scalar const A_aw = ALWAN_LITERAL(2.0) * rgb_aw.v[0] + rgb_aw.v[1] + rgb_aw.v[2] / ALWAN_LITERAL(20.0) - ALWAN_LITERAL(3.05) + ALWAN_ONE;

    /* Colour difference signals. */
    alwan_scalar const C1 = rgb_a.v[0] - rgb_a.v[1];
    alwan_scalar const C2 = rgb_a.v[1] - rgb_a.v[2];
    alwan_scalar const C3 = rgb_a.v[2] - rgb_a.v[0];
    alwan_scalar const C1w = rgb_aw.v[0] - rgb_aw.v[1];
    alwan_scalar const C2w = rgb_aw.v[1] - rgb_aw.v[2];
    alwan_scalar const C3w = rgb_aw.v[2] - rgb_aw.v[0];
    ...
    {
    alwan_scalar const e_s = hunt_eccentricity_v(h_deg);
    alwan_scalar const F_t = La / (La + ALWAN_LITERAL(0.1));

    /* Chromatic responses. */
    alwan_scalar const M_yb   = ALWAN_LITERAL(100.0) * (ALWAN_LITERAL(0.5) * (C2 - C3) / ALWAN_LITERAL(4.5)) *
                      (e_s * (ALWAN_LITERAL(10.0) / ALWAN_LITERAL(13.0)) * N_c * N_cb * F_t);
    alwan_scalar const M_rg   = ALWAN_LITERAL(100.0) * (C1 - (C2 / ALWAN_LITERAL(11.0))) *
                      (e_s * (ALWAN_LITERAL(10.0) / ALWAN_LITERAL(13.0)) * N_c * N_cb);
    alwan_scalar const M_yb_w = ALWAN_LITERAL(100.0) * (ALWAN_LITERAL(0.5) * (C2w - C3w) / ALWAN_LITERAL(4.5)) *
                      (e_s * (ALWAN_LITERAL(10.0) / ALWAN_LITERAL(13.0)) * N_c * N_cb * F_t);
    alwan_scalar const M_rg_w = ALWAN_LITERAL(100.0) * (C1w - (C2w / ALWAN_LITERAL(11.0))) *
                      (e_s * (ALWAN_LITERAL(10.0) / ALWAN_LITERAL(13.0)) * N_c * N_cb);
    alwan_scalar const M   = ALWAN_SQRT(M_yb * M_yb + M_rg * M_rg);
    alwan_scalar const M_w = ALWAN_SQRT(M_yb_w * M_yb_w + M_rg_w * M_rg_w);

    /* Saturation. */
    alwan_scalar const rgb_a_sum = rgb_a.v[0] + rgb_a.v[1] + rgb_a.v[2];
    ...
    {
    alwan_scalar const L_AS_226 = L_AS / ALWAN_LITERAL(2.26);
    alwan_scalar const five_LAS = ALWAN_LITERAL(5.0) * L_AS_226;
    alwan_scalar const j = ALWAN_LITERAL(0.00001) / (five_LAS + ALWAN_LITERAL(0.00001));
    alwan_scalar const F_LS = ALWAN_LITERAL(3800.0) * j * j * five_LAS +
    ...
    alwan_scalar const S_S_w   = S_p   / ALWAN_SELECT(ALWAN_ABS(S_w_p) > ALWAN_LITERAL(1e-12), S_w_p, ALWAN_ONE);
    alwan_scalar const S_S_w_w = ALWAN_ONE;
    alwan_scalar const B_S   = ALWAN_LITERAL(0.5) / (ALWAN_ONE + ALWAN_LITERAL(0.3) * ALWAN_POW(alwan_max(five_LAS * S_S_w, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.3))) +
                     ALWAN_LITERAL(0.5) / (ALWAN_ONE + ALWAN_LITERAL(5.0) * five_LAS);
    alwan_scalar const B_S_w = ALWAN_LITERAL(0.5) / (ALWAN_ONE + ALWAN_LITERAL(0.3) * ALWAN_POW(alwan_max(five_LAS * S_S_w_w, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.3))) +
                     ALWAN_LITERAL(0.5) / (ALWAN_ONE + ALWAN_LITERAL(5.0) * five_LAS);
    alwan_scalar const A_S   = (hunt_f_n_v(F_LS * S_S_w)   * ALWAN_LITERAL(3.05) * B_S)   + ALWAN_LITERAL(0.3);
    alwan_scalar const A_S_w = (hunt_f_n_v(F_LS * S_S_w_w) * ALWAN_LITERAL(3.05) * B_S_w) + ALWAN_LITERAL(0.3);
    alwan_scalar const root  = ALWAN_SQRT(ALWAN_ONE + ALWAN_LITERAL(0.3) * ALWAN_LITERAL(0.3));
    alwan_scalar const A     = N_bb * (A_a  - ALWAN_ONE + A_S   - ALWAN_LITERAL(0.3) + root);
    alwan_scalar const A_w   = N_bb * (A_aw - ALWAN_ONE + A_S_w - ALWAN_LITERAL(0.3) + root);

    /* Brightness. */
    {
    alwan_scalar const N_1 = ALWAN_POW(alwan_max(ALWAN_LITERAL(7.0) * A_w, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.5)) /
                   (ALWAN_LITERAL(5.33) * ALWAN_POW(alwan_max(N_b, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.13)));
    alwan_scalar const N_2 = (ALWAN_LITERAL(7.0) * A_w * ALWAN_POW(alwan_max(N_b, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.362))) / ALWAN_LITERAL(200.0);
    alwan_scalar const Q   = ALWAN_POW(alwan_max(ALWAN_LITERAL(7.0) * (A   + (M   / ALWAN_LITERAL(100.0))), ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.6)) * N_1 - N_2;
    alwan_scalar const Q_w = ALWAN_POW(alwan_max(ALWAN_LITERAL(7.0) * (A_w + (M_w / ALWAN_LITERAL(100.0))), ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.6)) * N_1 - N_2;
    alwan_scalar const Q_w_safe = ALWAN_SELECT(ALWAN_ABS(Q_w) > ALWAN_LITERAL(1e-12), Q_w, ALWAN_ONE);
    alwan_scalar const Y_b_Y_w = Y_b / Yw_safe;
    alwan_scalar const Z = ALWAN_ONE + ALWAN_POW(alwan_max(Y_b_Y_w, ALWAN_LITERAL(0.0)), ALWAN_LITERAL(0.5));
