...
static alwan_f64 (*lut__resolve_eotf(alwan_transfer_function tf))(alwan_f64) {
    switch (tf) {
...
static alwan_f64 (*lut__resolve_oetf(alwan_transfer_function tf))(alwan_f64) {
    switch (tf) {
    case ALWAN_TF_LINEAR:     return alwan_linear_identity_f64;
...
static alwan_vec3 lut__apply_view(alwan_view_transform vt, alwan_vec3 rgb) {
    ...
    case ALWAN_VIEW_ACES_REC709:
        r.v[0] = alwan_saturate(alwan_aces_tonemap_f64_v(in.v[0]));
        r.v[1] = alwan_saturate(alwan_aces_tonemap_f64_v(in.v[1]));
        r.v[2] = alwan_saturate(alwan_aces_tonemap_f64_v(in.v[2]));
        return r;
    case ALWAN_VIEW_REINHARD_EXT:
        r = alwan_reinhard_extended_luma_f64_v(in, ALWAN_LITERAL(4.0));
        r.v[0] = alwan_saturate(r.v[0]);
        r.v[1] = alwan_saturate(r.v[1]);
        r.v[2] = alwan_saturate(r.v[2]);
        return r;
    case ALWAN_VIEW_UCHIMURA:
        r.v[0] = alwan_saturate(alwan_uchimura_default_f64_v(in.v[0]));
        r.v[1] = alwan_saturate(alwan_uchimura_default_f64_v(in.v[1]));
        r.v[2] = alwan_saturate(alwan_uchimura_default_f64_v(in.v[2]));
        return r;
    case ALWAN_VIEW_LOTTES:
        r = alwan_lottes_default_f64_v(in);
        r.v[0] = alwan_saturate(r.v[0]);
        r.v[1] = alwan_saturate(r.v[1]);
        r.v[2] = alwan_saturate(r.v[2]);
        return r;
    case ALWAN_VIEW_EXPOSURE:
        r = alwan_exposure_tonemap_rgb_f64_v(in, ALWAN_ZERO);
        r.v[0] = alwan_saturate(r.v[0]);
        r.v[1] = alwan_saturate(r.v[1]);
        r.v[2] = alwan_saturate(r.v[2]);
        return r;
    case ALWAN_VIEW_REINHARD_CALIBRATED:
        r.v[0] = alwan_saturate(alwan_reinhard_calibrated_f64_v(
                     in.v[0], ALWAN_LITERAL(0.18), ALWAN_LITERAL(0.18), ALWAN_LITERAL(4.0)));
        r.v[1] = alwan_saturate(alwan_reinhard_calibrated_f64_v(
                     in.v[1], ALWAN_LITERAL(0.18), ALWAN_LITERAL(0.18), ALWAN_LITERAL(4.0)));
        r.v[2] = alwan_saturate(alwan_reinhard_calibrated_f64_v(
                     in.v[2], ALWAN_LITERAL(0.18), ALWAN_LITERAL(0.18), ALWAN_LITERAL(4.0)));
        return r;
    case ALWAN_VIEW_KHRONOS_PBR_NEUTRAL:
        return alwan_khronos_pbr_neutral_f64_v(in);
    default:
...
int alwan_bake_3dlut(alwan_f64 *out, int size,
                     alwan_ctx *ctx,
                     alwan_rgb_space_desc const *src_space,
                     alwan_rgb_space_desc const *dst_space) {
    ...
    alwan_f64 (*eotf_fn)(alwan_f64) = lut__resolve_eotf(src_space->eotf);
    alwan_f64 (*oetf_fn)(alwan_f64) = lut__resolve_oetf(dst_space->oetf);
...
int alwan_bake_3dlut_view(alwan_f64 *out, int size,
                          alwan_ctx *ctx,
                          alwan_rgb_space_desc const *src_space,
                          alwan_rgb_space_desc const *dst_space,
...
int alwan_lut3d_to_2d(alwan_f64 *out,
...
int alwan_lut2d_to_3d(alwan_f64 *out,
...
int alwan_bake_2dlut(alwan_f64 *out, int size,
                     alwan_ctx *ctx,
                     alwan_rgb_space_desc const *src_space,
                     alwan_rgb_space_desc const *dst_space) {
    if (!out || !src_space || !dst_space || size < 2 || size > 256) {
    ...
    int const w = size * size;
    alwan_f64 const inv = ALWAN_LITERAL(1.0) / (alwan_f64)(size - 1);
    ...
    for (int py = 0; py < size; py++) {           /* G axis */
        for (int px = 0; px < w; px++) {            /* B*size + R */
            ...
            alwan_f64 rv = (alwan_f64)r * inv;
            alwan_f64 gv = (alwan_f64)g * inv;
...
int alwan_lut1d_sample(alwan_f64 *result,
                        alwan_f64 const *lut,
                        alwan_f64 t,
                        int size) {
    if (!result || !lut || size < 2) return ALWAN_E_INVALID;

    *result = alwan_lut1d_sample_f64_v(lut, t, size);
    return ALWAN_OK;
}
