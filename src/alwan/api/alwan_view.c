...
#include "../data/matrices/aces_ap1_to_ap0.csv"
    ...
    /* Transform to AP0 */
    alwan_f64 ap0_r = ap1_to_ap0[0] * rgb_in[0] + ap1_to_ap0[1] * rgb_in[1] + ap1_to_ap0[2] * rgb_in[2];
    alwan_f64 ap0_g = ap1_to_ap0[3] * rgb_in[0] + ap1_to_ap0[4] * rgb_in[1] + ap1_to_ap0[5] * rgb_in[2];
    alwan_f64 ap0_b = ap1_to_ap0[6] * rgb_in[0] + ap1_to_ap0[7] * rgb_in[1] + ap1_to_ap0[8] * rgb_in[2];

    /* Apply simplified RRT tone curve */
    alwan_f64 rrt_r = alwan_aces_tonemap_f64_v(ap0_r);
    alwan_f64 rrt_g = alwan_aces_tonemap_f64_v(ap0_g);
    alwan_f64 rrt_b = alwan_aces_tonemap_f64_v(ap0_b);
    ...
    ALWAN_DIAG_PUSH
    ALWAN_DIAG_DISABLE_FLOAT_CONV
    static alwan_f64 const odt_matrix[9] = {
...
static void agx_base_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    /* Apply inset matrix */
    alwan_f64 inset_r = agx_inset_matrix[0] * rgb_in[0] + agx_inset_matrix[1] * rgb_in[1] + agx_inset_matrix[2] * rgb_in[2];
    alwan_f64 inset_g = agx_inset_matrix[3] * rgb_in[0] + agx_inset_matrix[4] * rgb_in[1] + agx_inset_matrix[5] * rgb_in[2];
    alwan_f64 inset_b = agx_inset_matrix[6] * rgb_in[0] + agx_inset_matrix[7] * rgb_in[1] + agx_inset_matrix[8] * rgb_in[2];

    /* AgX log-encode + curve */
    alwan_f64 const agx_min = ALWAN_LITERAL(-12.47393);
    alwan_f64 const agx_max = ALWAN_LITERAL(  4.026069);

    alwan_f64 curve_r = alwan_agx_curve_f64_v(alwan_agx_log_encode_f64_v(inset_r, agx_min, agx_max));
    alwan_f64 curve_g = alwan_agx_curve_f64_v(alwan_agx_log_encode_f64_v(inset_g, agx_min, agx_max));
    alwan_f64 curve_b = alwan_agx_curve_f64_v(alwan_agx_log_encode_f64_v(inset_b, agx_min, agx_max));
...
static void agx_punchy_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 graded = alwan_agx_punchy_grade_f64_v(base);
...
static void agx_golden_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 base;
    base.v[0] = rgb_out[0]; base.v[1] = rgb_out[1]; base.v[2] = rgb_out[2];
    alwan_vec3 graded = alwan_agx_golden_grade_f64_v(base);
    rgb_out[0] = graded.v[0]; rgb_out[1] = graded.v[1]; rgb_out[2] = graded.v[2];
}
...
static void khronos_pbr_neutral_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 out_v = alwan_khronos_pbr_neutral_f64_v(in_v);
    rgb_out[0] = out_v.v[0];
    rgb_out[1] = out_v.v[1];
    rgb_out[2] = out_v.v[2];
}
...
static void reinhard_ext_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    alwan_vec3 in_v = {{rgb_in[0], rgb_in[1], rgb_in[2]}};
    alwan_vec3 out_v = alwan_reinhard_extended_luma_f64_v(in_v, ALWAN_LITERAL(4.0));
...
static void lottes_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 out_v = alwan_lottes_default_f64_v(in_v);
    rgb_out[0] = alwan_saturate(out_v.v[0]);
    rgb_out[1] = alwan_saturate(out_v.v[1]);
    rgb_out[2] = alwan_saturate(out_v.v[2]);
}
...
static void tony_mcmapface_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 out_v = alwan_tony_mcmapface_f64_v(in_v);
    rgb_out[0] = alwan_saturate(out_v.v[0]);
    rgb_out[1] = alwan_saturate(out_v.v[1]);
    rgb_out[2] = alwan_saturate(out_v.v[2]);
}
...
static void exposure_transform(alwan_f64 const *rgb_in, alwan_f64 *rgb_out) {
    ...
    alwan_vec3 out_v = alwan_exposure_tonemap_rgb_f64_v(in_v, ALWAN_ZERO);
    rgb_out[0] = alwan_saturate(out_v.v[0]);
    rgb_out[1] = alwan_saturate(out_v.v[1]);
    rgb_out[2] = alwan_saturate(out_v.v[2]);
}
