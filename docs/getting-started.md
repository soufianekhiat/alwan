...
Most conversions follow the pattern: `source_to_dest(input, config, output, count, in_stride, out_stride)`
...
alwan_xyz_to_lab(xyz_array, &alwan_d65_xyz, lab_array, 100,
...
// Process only RGB, skip alpha
alwan_oetf_apply(ctx, "srgb", rgba, 100,
...
alwan_eotf_apply(ctx, "srgb",
                 srgb_encoded, 1920 * 1080, 3 * sizeof(float),
                 srgb_linear, 3 * sizeof(float));
```
...
alwan_rgb_convert(ctx, "srgb", "bt2020",
                  srgb_data, pixel_count, 3 * sizeof(float),
                  bt2020_data, 3 * sizeof(float));
```
...
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                ...
                xyz_d50, 100, sizeof(alwan_vec3),
                xyz_d65, sizeof(alwan_vec3));
```
...
alwan_view_transform_apply(ctx, "agx",
                           hdr_rgb, 1920 * 1080, 3 * sizeof(float),
                           display_rgb, 3 * sizeof(float));
```
...
alwan_xyz_to_lab(&xyz, NULL, &lab, 1, 0, 0);  // Error!
```
