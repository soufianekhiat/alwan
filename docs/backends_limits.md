...
For functions that genuinely want out‑parameters, the house idiom is
...
So embedded tables are **not fundamentally blocked** on GPU. What actually keeps
the LUT‑backed features C‑only today is *structure*, not data: the LUT samplers
...
- **GLSL**: no `double` → `alwan_f64` aliases to `float`; `atan2(y,x)` = `atan(y,x)`;
  `ALWAN_INLINE` is empty (file‑scope functions).
- **Halide**: real `double`; kernels build symbolic `Expr`s; branchless
  `ALWAN_SELECT` → `Halide::select` (raw scalar `if` is not allowed).
