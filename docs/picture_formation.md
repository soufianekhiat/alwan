...
and still all-`ok` after `REL`/`VEIL` joined — 15 columns). See the `COMPLETE` section below for how
they clear `HEMI_ABS`'s `RATIO`.

`REL`/`VEIL` measured all-`ok` across the family: every method maps the neutral veil scene
monotonically, so polarity survives; the honest caveat is the *margin* — at `T = 0.1` the formed
...
2. **A desaturation floor instead of a low rail.** Cap the purity-keep at `KEEPMAX < 1`, drawing every
   ...
   not per-channel, so the `SPAN`/`RATIO`/`RAIL` conflict dissolves.
...
- **`COMPLETE`** (`19.85 / 0.861 / 0.005`) widens the window to the isophote-through-highlight maximum
  ...
  constants are a *design choice*, not a solver output.
...
metric, but through tonescale width, not the staggered per-channel saturation of true channel
...
integration delivers — which is now a question about the metric, not about whether a complete
operator exists.
...
construction*, not by a penalty or a projection — no isotonic pooling, no staircase); because it is
an analytic function of the base it manufactures no plateaus. It measures **11-of-13**: the same
row as `HEMISPHERE_ABS` except it loses `INVR` — it is a *spatial* operator, so a patch's output
depends on its surround (the same gap `HEMISPHERE` has, and the reason the veil operators are
`INVR` `xacc`) — and it inherits `HEMISPHERE_ABS`'s `RATIO` `xacc` (the ratio-preserving scale does
...
carrier-monotone and artifact-free. What it is **not** is a shortcut to channel integration:
...
rail and reports the honest number.) `WARP` is therefore the carrier family's local-contrast option,
not a resolution of the open question — that resolution is `COMPLETE`, above.
...
structure — and with it a second constraint set. These apply to the **experimental trio** in
`src/alwan/experimental/` (`alwan_picture_form_global_exp`, `alwan_picture_form_local_exp`,
`alwan_picture_form_evidence`), tested in `tests/100_formation_experimental.c`:

| key | constraint | status in the trio |
|---|---|---|
...
| *rivalry preservation* | competing scene organisations stay **soft** — genuine ambiguity is represented, not forced to a single labelling | **partial**: the evidence field is a continuous distribution `P(C)/P(T)/P(O)` (never hard labels); its entropy is not yet exposed |
