...
surface (precision and `_map_*` variants folded together), **101 have no entry in
`docs/api/`**.

Three are closed: the table sampling family now has [api/tables.md](api/tables.md),
`alwan_gamut_map_spatial` plus the bulk gamut forms are in
[api/gamut.md](api/gamut.md), and the LUT bake, interchange and sampling surface
(`alwan_bake_*`, `alwan_lut*`, `alwan_cube_*`, `alwan_clf_*`) is in
[api/luts.md](api/luts.md).
