...
> **⚠️ Note:** This documentation describes the planned API. The actual API in v0.1-alpha may differ. See [alwan.h](../../src/alwan/alwan.h) for current implementation.
...
**Parameters:** (similar to `alwan_eotf_apply`)
...
alwan_oetf_apply(ctx, "srgb",
                 (alwan_vec3*)linear, 1000, 12,
                 (alwan_vec3*)encoded, 12);
```
...
**Example:**
```c
// HDR scene to SDR display
alwan_vec3 hdr_scene[1920 * 1080];  // [0, ∞)
alwan_vec3 sdr_display[1920 * 1080]; // [0, 1]

alwan_view_transform_apply(ctx, "agx",
                           hdr_scene, 1920 * 1080, 12,
                           sdr_display, 12);

// Ready for display encoding
alwan_oetf_apply(ctx, "srgb",
                 sdr_display, 1920 * 1080, 12,
                 sdr_display, 12);
```
...
// 2. Decode to linear
float *linear = malloc(...);
alwan_eotf_apply(ctx, "srgb", encoded, count, 12, linear, 12);

// 3. Process in linear space
apply_color_grading(linear, count);

// 4. Encode back to sRGB
alwan_oetf_apply(ctx, "srgb", linear, count, 12, encoded, 12);
...
// Decode to linear
float *linear = malloc(...);
alwan_eotf_apply(ctx, "clog", clog, count, 12, linear, 12);
...
float *srgb = malloc(...);
alwan_oetf_apply(ctx, "srgb", linear, count, 12, srgb, 12);
```
...
alwan_oetf_apply(ctx, "srgb", &original, 1, 0, &encoded, 0);
alwan_eotf_apply(ctx, "srgb", &encoded, 1, 0, &decoded, 0);
