...
> **⚠️ Note:** This documentation describes the planned API. The actual API in v0.1-alpha may differ. See [alwan.h](../../src/alwan/alwan.h) for current implementation. Runtime data loading (`runtime_data_root`) is not yet implemented.
...
for (int i = 0; i < n; i++) {
    ...
    alwan_rgb_convert(shared_ctx, "srgb", "bt2020", &rgb[i], 1, 0, &out[i], 0);
}
