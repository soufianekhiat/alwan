...
Functions for color appearance models (CIECAM02, CAM16) that predict color perception under different viewing conditions.
...
// Convert display XYZ → CAM16 → print XYZ
alwan_scalar J, C, h;
alwan_cam16_forward(ctx, &display_xyz, &dim_office, &J, &C, &h, ...);
alwan_cam16_reverse(ctx, J, C, h, &bright_print, &print_xyz);
```
...
```c
// ΔE in CAM16-UCS space
alwan_scalar J1, M1, h1, J2, M2, h2;
alwan_cam16_forward(ctx, &xyz1, &vc, &J1, ..., &M1, &h1);
alwan_cam16_forward(ctx, &xyz2, &vc, &J2, ..., &M2, &h2);

alwan_scalar J1p, a1p, b1p, J2p, a2p, b2p;
alwan_cam16_to_ucs(J1, M1, h1, &J1p, &a1p, &b1p);
alwan_cam16_to_ucs(J2, M2, h2, &J2p, &a2p, &b2p);
