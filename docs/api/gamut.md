...
alwan_gamut_map_perceptual(ctx, &bt2020_rgb,
...
```c
alwan_result alwan_gamut_volume(
    alwan_ctx *ctx,
    alwan_rgb_space space,
    alwan_scalar *volume
);
...
alwan_gamut_volume(ctx, ALWAN_RGB_SPACE_SRGB, &srgb_volume);
alwan_gamut_volume(ctx, ALWAN_RGB_SPACE_BT2020, &bt2020_volume);
...
```c
bool alwan_is_in_pointer_gamut(
    const alwan_vec2 *xy
);
```
...
alwan_vec3 srgb_rgb;
alwan_rgb_convert(ctx, ALWAN_RGB_SPACE_PROPHOTO_RGB,
                  ALWAN_RGB_SPACE_SRGB,
                  &prophoto_rgb, &srgb_rgb);

// Check if mapping needed
if (!alwan_is_in_gamut(ctx, &srgb_rgb, ALWAN_RGB_SPACE_SRGB)) {
    // Apply perceptual mapping
    alwan_gamut_map_perceptual(ctx, &srgb_rgb,
                                ALWAN_RGB_SPACE_SRGB,
                                &srgb_rgb);
}
...
for (int i = 0; i < 4; i++) {
    alwan_scalar volume;
    alwan_gamut_volume(ctx, space_ids[i], &volume);
    printf("  %s: %.0f\n", spaces[i], volume);
}
```
...
void map_image_to_srgb(
    ...
    for (int i = 0; i < pixel_count; i++) {
        if (!alwan_is_in_gamut(ctx, &rgb_data[i],
                               ALWAN_RGB_SPACE_SRGB)) {
            alwan_gamut_map_perceptual(ctx, &rgb_data[i],
                                        ALWAN_RGB_SPACE_SRGB,
                                        &rgb_data[i]);
        }
    }
