...
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d50_xyz, &alwan_d65_xyz,
                xyz_in, count, stride_in, xyz_out, stride_out);
```
...
```c
alwan_xyz_adapt(ctx, ALWAN_CAT_CAT02,
                &src_white, &dst_white,
                xyz_in, count, stride_in, xyz_out, stride_out);
```
...
```c
alwan_xyz_adapt(ctx, ALWAN_CAT_CAT16,
                &src_white, &dst_white,
                xyz_in, count, stride_in, xyz_out, stride_out);
```
...
```c
alwan_xyz_adapt(ctx, ALWAN_CAT_XYZ_SCALING,
                &src_white, &dst_white,
                xyz_in, count, stride_in, xyz_out, stride_out);
```
...
// Convert to screen (D65)
alwan_vec3 xyz_screen[1000];
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d50_xyz, &alwan_d65_xyz,
                xyz_print, 1000, 12, xyz_screen, 12);
```
...
alwan_vec3 xyz_d65;
alwan_rgb_to_xyz_space(ctx, "adobe_rgb", &adobe_rgb, &xyz_d65, 1, 0, 0);

// 2. Adapt D65 → D50
alwan_vec3 xyz_d50;
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d65_xyz, &alwan_d50_xyz,
                &xyz_d65, 1, 0, &xyz_d50, 0);

// 3. XYZ (D50) → ProPhoto RGB
alwan_vec3 prophoto_rgb;
alwan_xyz_to_rgb_space(ctx, "prophoto_rgb", &xyz_d50, &prophoto_rgb, 1, 0, 0);
```
...
```c
// Adapt from fluorescent F2 to D65
...
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d65_xyz, &alwan_d50_xyz,
                &original, 1, 0, &adapted, 0);

alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d50_xyz, &alwan_d65_xyz,
                &adapted, 1, 0, &recovered, 0);
...
```c
// D65 → D50 (screen to print)
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d65_xyz, &alwan_d50_xyz,
                xyz_in, count, stride, xyz_out, stride);

// D50 → D65 (print to screen)
alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
                &alwan_d50_xyz, &alwan_d65_xyz,
                xyz_in, count, stride, xyz_out, stride);
```
...
alwan_result r = alwan_xyz_adapt(ctx, ALWAN_CAT_BRADFORD,
