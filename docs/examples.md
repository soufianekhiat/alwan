...
int main(void) {
    ...
    alwan_vec3 srgb;
    alwan_rgb_convert(ctx, "bt2020", "srgb", &bt2020, 1, 0, &srgb, 0);
...
int main(void) {
    ...
    alwan_eotf_apply(ctx, "srgb",
                     srgb_encoded->data, pixel_count, 3 * sizeof(float),
                     linear.data, 3 * sizeof(float));
    ...
    alwan_oetf_apply(ctx, "srgb",
                     linear.data, pixel_count, 3 * sizeof(float),
                     srgb_encoded->data, 3 * sizeof(float));
...
int main(void) {
    ...
    alwan_eotf_apply(ctx, "srgb",
                     rgba, width * height, 4 * sizeof(float),
                     rgba, 4 * sizeof(float));
...
int main(void) {
    ...
    alwan_view_transform_apply(ctx, "agx",
                               hdr_colors, count, sizeof(alwan_vec3),
                               display_rgb, sizeof(alwan_vec3));
...
int main(void) {
    ...
    alwan_xyz_to_ictcp(&xyz1, "pq", &ictcp1, 1, 0, 0);
    alwan_xyz_to_ictcp(&xyz2, "pq", &ictcp2, 1, 0, 0);
...
void shift_hue(alwan_vec3 *rgb, float hue_shift_degrees) {
    ...
    alwan_rgb_to_xyz(rgb, &xyz, 1, 0, 0);
    alwan_xyz_to_lab(&xyz, &alwan_d65_xyz, &lab, 1, 0, 0);
    alwan_lab_to_lch(&lab, &lch, 1, 0, 0);
    ...
    alwan_lch_to_lab(&lch, &lab, 1, 0, 0);
    alwan_lab_to_xyz(&lab, &alwan_d65_xyz, &xyz, 1, 0, 0);
    alwan_xyz_to_rgb(&xyz, rgb, 1, 0, 0);
}
...
int main(void) {
    ...
    alwan_rgb_to_xyz(&rgb1, &xyz1, 1, 0, 0);
    alwan_rgb_to_xyz(&rgb2, &xyz2, 1, 0, 0);
    alwan_xyz_to_lab(&xyz1, &alwan_d65_xyz, &lab1, 1, 0, 0);
    alwan_xyz_to_lab(&xyz2, &alwan_d65_xyz, &lab2, 1, 0, 0);

    // Calculate ΔE2000
    alwan_scalar delta_e;
    alwan_delta_e_2000(&lab1, &lab2, &delta_e, 1, 0, 0);
...
int find_closest_color(const alwan_vec3 *target_lab,
    ...
    for (int i = 0; i < palette_size; i++) {
        alwan_scalar delta_e;
        alwan_delta_e_2000(target_lab, &palette_lab[i], &delta_e, 1, 0, 0);
...
int main(void) {
    ...
    alwan_rgb_to_xyz(&target_rgb, &target_xyz, 1, 0, 0);
    alwan_xyz_to_lab(&target_xyz, &alwan_d65_xyz, &target_lab, 1, 0, 0);

    alwan_vec3 palette_lab[palette_size];
    for (int i = 0; i < palette_size; i++) {
        alwan_vec3 xyz;
        alwan_rgb_to_xyz(&palette_rgb[i], &xyz, 1, 0, 0);
        alwan_xyz_to_lab(&xyz, &alwan_d65_xyz, &palette_lab[i], 1, 0, 0);
    }
...
int main(void) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < width * height; i++) {
            ...
            alwan_rgb_convert(ctx, "srgb", "bt2020", src, 1, 0, dst, 0);
        }
    }
...
int main(void) {
    ...
    result = alwan_rgb_convert(ctx, "srgb", "invalid_space",
                               &rgb, 1, 0, &converted, 0);
    ...
    result = alwan_rgb_convert(ctx, "srgb", "bt2020",
                               NULL, 1, 0, &converted, 0);
