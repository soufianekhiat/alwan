...
Input image `I`: pixels `x_i in R^3` (linear RGB records), some outside the cube
`[0,1]^3` (or `[0, peak]^3` for HDR).
...
- **P4, bounded:** `x'_i in [0,1]^3`; gradients collapse toward the bounds.
...
```
E(u) =  Sigma_i  lambda_i * (u_i - x_i)^2                          [fidelity, spatial weight]
     +  beta * Sigma_{i~j} w_ij * ((u_i - u_j) - c * g_ij)^2      [gradient preservation]
```
subject to
```
u_i in [0,1]                                              (box / gamut)
(u_i - u_j) * sign(g_ij) >= 0   for all i~j                     (P1, monotonicity)
```
...
### 2.1 The spatial spread weight `lambda_i` (the `s` knob)

Let `Omega = { i : x_i not in [0,1]^3 }` be the out-of-gamut set.
- `D_i` = spatial Euclidean (L2) distance from pixel `i` to the nearest pixel in
  `Omega` (exact EDT, deterministic, O(n)).
- Normalize: `D_hat_i = min(D_i / R, 1)`, `R` = spatial reach (default: image
  diagonal, or a user radius in pixels).
- Weight:
  ```
  lambda_i = lambda0 * smoothstep( (D_hat_i - (1 - s)) / feather )
  ```
  - `s = 1`: threshold at 0, so every in-gamut pixel is locked and only `Omega`
...
- `Omega` pixels themselves get `lambda_i = 0` (fully free; they must move).
...
- `beta` = structure-vs-fidelity balance (default ~ a few x lambda0).
...
- **Edge coupling:** `w_ij <- w_ij * f(|z_i - z_j|)`, `f -> 0` across a depth jump
  (`exp(-(Deltaz/sigma)^2)` or a hard threshold). Across an occlusion boundary there is
...
overlapping layers) is not implemented (section 6); there depth cannot cut pixels spatially, it
...
Notes:
- Convex feasible set (box intersect half-spaces) is non-empty (`u == const` or the
...
- **In-gamut:** every output record in [0, peak]^3.
...
- **`s` endpoints:** `s=1` leaves in-gamut pixels unchanged (only `Omega` moves);
...
```
alpha = (p - q)/(a - b),   t = (a*q - b*p)/(a - b)
valid layer  <=>  0 <= alpha <= 1  and  t >= 0     (order-preserving affine contraction)
