...
> **⚠️ Documentation Status:** This documentation represents the planned/aspirational API design and may not fully match the current implementation. The library is under active development (v0.1-alpha). Please refer to [alwan.h](../src/alwan/alwan.h) for the actual current API.

> **⚠️ Runtime Data Loading:** Runtime data loading mode (`ALWAN_EMBED_DATA=0`) is not yet fully implemented. Currently, only embedded mode (`ALWAN_EMBED_DATA=1`, default) is supported.
...
### Parameter Order
Functions follow consistent parameter ordering:
1. Context (if required)
2. Configuration/descriptor parameters
3. Input data pointer
4. Count (for bulk operations)
5. Input stride
...
alwan_oetf_apply(ctx, "srgb", rgba_data, 100, 16, rgba_data, 16);
//                                            ^^               ^^
//                          stride in bytes (4 floats = 16 bytes)
```
...
for (int i = 0; i < n; i++) {
    alwan_xyz_to_lab(&xyz[i], &d65, &lab[i], 1, 0, 0);
}
...
**Fast:**
```c
alwan_xyz_to_lab(xyz, &d65, lab, 1000, sizeof(alwan_vec3), sizeof(alwan_vec3));
```
