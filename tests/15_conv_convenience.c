...
static int test_hsv_forward(void) {
    ...
    for (size_t i = 0; i < NUM_TEST_COLORS; i++) {
        ...
        alwan_vec3 hsv;
        int status = alwan_rgb_to_hsv((alwan_hsv *)&hsv, (alwan_rgb const *)&rgb);
...
static int test_hsl_forward(void) {
    ...
    for (size_t i = 0; i < NUM_TEST_COLORS; i++) {
        ...
        alwan_vec3 hsl;
        int status = alwan_rgb_to_hsl((alwan_hsl *)&hsl, (alwan_rgb const *)&rgb);
...
static int test_cmy_conversions(void) {
    ...
    for (size_t i = 0; i < NUM_TEST_COLORS; i++) {
        ...
        alwan_scalar diff_g = ALWAN_ABS(rgb_recon.g - rgb.g);
        alwan_scalar diff_b = ALWAN_ABS(rgb_recon.b - rgb.b);
...
static int test_cmyk_conversions(void) {
    ...
    for (size_t i = 0; i < NUM_TEST_COLORS; i++) {
        ...
        alwan_scalar diff_k = ALWAN_ABS(k - exp_k);
        ...
        alwan_scalar diff_c2 = ALWAN_ABS(cmy_recon.c - cmy.c);
        alwan_scalar diff_m2 = ALWAN_ABS(cmy_recon.m - cmy.m);
        alwan_scalar diff_y2 = ALWAN_ABS(cmy_recon.y - cmy.y);
