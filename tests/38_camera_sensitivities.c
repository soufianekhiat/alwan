...
static int test_camera_sensitivity_loading(void) {
    ...
    status = alwan_spd_camera_sensitivity(&r_sens, &g_sens, &b_sens, ctx, ALWAN_CAMERA_NIKON_5100);
    ...
    status = alwan_spd_create(&r_sens, ctx, ALWAN_LITERAL(360.0), ALWAN_LITERAL(830.0), 471);
    TEST_ASSERT(status == ALWAN_OK, "Failed to create R SPD");
    status = alwan_spd_create(&g_sens, ctx, ALWAN_LITERAL(360.0), ALWAN_LITERAL(830.0), 471);
    TEST_ASSERT(status == ALWAN_OK, "Failed to create G SPD");
    status = alwan_spd_create(&b_sens, ctx, ALWAN_LITERAL(360.0), ALWAN_LITERAL(830.0), 471);
    ...
    status = alwan_spd_camera_sensitivity(&r_sens, &g_sens, &b_sens, ctx, ALWAN_CAMERA_SIGMA_SDMERILL);
...
static int test_xyz_from_spd_camera(void) {
    ...
    status = alwan_xyz_from_spd_camera(&rgb_sigma, ctx, &d65, NULL, ALWAN_CAMERA_SIGMA_SDMERILL,
                                        ALWAN_INTEGRATE_TRAPEZOID);
