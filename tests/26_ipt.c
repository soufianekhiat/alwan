...
#include "reference_values/test_xyz_ipt_pairs.csv"
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            alwan_scalar diff = ALWAN_FABS(ipt_comp_arr[j] - ipt_exp_arr[j]);
            if (diff > ipt_tol) {
...
static int test_ipt_iptch_round_trip(void) {
    ...
    alwan_ipt_to_iptch(&ipt, &iptch);
    TEST_ASSERT(ALWAN_FABS(iptch.I - ALWAN_LITERAL(0.5)) < ALWAN_TEST_TOLERANCE, "I mismatch");
    TEST_ASSERT(ALWAN_FABS(iptch.C) < ALWAN_TEST_TOLERANCE, "C should be 0");

    alwan_iptch_to_ipt(&iptch, &ipt_out);
    alwan_scalar ipt_arr[3] = {ipt.I, ipt.P, ipt.T};
    alwan_scalar ipt_out_arr[3] = {ipt_out.I, ipt_out.P, ipt_out.T};
    for (int i = 0; i < 3; i++) {
        TEST_ASSERT(ALWAN_FABS(ipt_out_arr[i] - ipt_arr[i]) < ALWAN_TEST_TOLERANCE,
    ...
    for (int i = 0; i < 3; i++) {
        alwan_scalar diff = ALWAN_FABS(ipt_out_arr[i] - ipt_arr[i]);
