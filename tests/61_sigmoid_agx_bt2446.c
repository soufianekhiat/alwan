...
static int test_sigmoid_roundtrip(void) {
    alwan_f64 pivot = ALWAN_LITERAL(0.18);
    alwan_f64 slope = ALWAN_LITERAL(1.0);
    alwan_f64 toe = ALWAN_LITERAL(1.5);
    alwan_f64 shoulder = ALWAN_LITERAL(1.5);
    ...
    for (int i = 0; i < 6; i++) {
        alwan_f64 x = test_values[i];
        alwan_f64 y = alwan_sigmoid_f64_v(x, pivot, slope, toe, shoulder);
        alwan_f64 x_rt = alwan_sigmoid_inv_f64_v(y, pivot, slope, toe, shoulder);
...
static int test_sigmoid_monotonic(void) {
    alwan_f64 pivot = ALWAN_LITERAL(0.18);
    alwan_f64 slope = ALWAN_LITERAL(1.0);
    alwan_f64 toe = ALWAN_LITERAL(2.0);
    alwan_f64 shoulder = ALWAN_LITERAL(2.0);

    alwan_f64 prev = alwan_sigmoid_f64_v(ALWAN_LITERAL(0.0), pivot, slope, toe, shoulder);
    for (int i = 1; i <= 100; i++) {
        alwan_f64 x = (alwan_f64)i / ALWAN_LITERAL(100.0);
        alwan_f64 y = alwan_sigmoid_f64_v(x, pivot, slope, toe, shoulder);
...
static int test_agx_basic(void) {
    alwan_f64 rgb_in[3] = { ALWAN_LITERAL(0.18), ALWAN_LITERAL(0.18), ALWAN_LITERAL(0.18) };
    alwan_f64 rgb_out[3];

    int status = alwan_view_transform_apply(
        rgb_out, NULL, ALWAN_VIEW_AGX, rgb_in, 1,
        3 * sizeof(alwan_f64), 3 * sizeof(alwan_f64));
...
static int test_agx_golden(void) {
    alwan_f64 rgb_in[3] = { ALWAN_LITERAL(0.5), ALWAN_LITERAL(0.3), ALWAN_LITERAL(0.1) };
    alwan_f64 rgb_out[3];
...
static int test_bt2446a_roundtrip(void) {
    ...
    for (int i = 0; i < 5; i++) {
        alwan_f64 y_hdr = test_values[i];
        alwan_f64 y_sdr = alwan_bt2446a_forward_f64_v(y_hdr, L_hdr, L_sdr);
        alwan_f64 y_rt = alwan_bt2446a_inverse_f64_v(y_sdr, L_hdr, L_sdr);
...
static int test_bt2446a_api(void) {
    alwan_f64 rgb_in[3] = { ALWAN_LITERAL(0.5), ALWAN_LITERAL(0.5), ALWAN_LITERAL(0.5) };
    alwan_f64 rgb_out[3];
