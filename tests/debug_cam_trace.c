...
#include "reference_values/cam_viewing_conditions.csv"
    ...
    alwan_scalar F = 1.0;
    alwan_scalar D = F * (1.0 - 1.0/3.6 * exp((-L_A - 42.0) / 92.0));
    printf("D: %.20e\n", D);

    /* Step 4: Chromatic adaptation */
    alwan_scalar D_R = D * (XYZ_w.y / rgb_cat_w.v[0]) + 1.0 - D;
    alwan_scalar D_G = D * (XYZ_w.y / rgb_cat_w.v[1]) + 1.0 - D;
    alwan_scalar D_B = D * (XYZ_w.y / rgb_cat_w.v[2]) + 1.0 - D;
    ...
    alwan_scalar scaled_R = FL * hpe.v[0] / 100.0;
    alwan_scalar scaled_G = FL * hpe.v[1] / 100.0;
    alwan_scalar scaled_B = FL * hpe.v[2] / 100.0;
    printf("scaled: [%.20e, %.20e, %.20e]\n", scaled_R, scaled_G, scaled_B);

    alwan_scalar pow_R = pow(fabs(scaled_R), 0.42);
    alwan_scalar pow_G = pow(fabs(scaled_G), 0.42);
    alwan_scalar pow_B = pow(fabs(scaled_B), 0.42);
    printf("pow: [%.20e, %.20e, %.20e]\n", pow_R, pow_G, pow_B);

    alwan_scalar R_a = 400.0 * pow_R / (27.13 + pow_R) + 0.1;
    alwan_scalar G_a = 400.0 * pow_G / (27.13 + pow_G) + 0.1;
    alwan_scalar B_a = 400.0 * pow_B / (27.13 + pow_B) + 0.1;
    printf("RGB_a: [%.20e, %.20e, %.20e]\n", R_a, G_a, B_a);

    /* Step 8: Opponent signals */
    alwan_scalar a = R_a - 12.0 * G_a / 11.0 + B_a / 11.0;
    alwan_scalar b = (R_a + G_a - 2.0 * B_a) / 9.0;
    printf("a: %.20e\n", a);
    printf("b: %.20e\n", b);

    /* Step 9: Hue angle */
    alwan_scalar h_rad = atan2(b, a);
    alwan_scalar h = h_rad * 180.0 / 3.14159265358979323846;
    if (h < 0) h += 360.0;
    printf("h_rad: %.20e\n", h_rad);
    printf("h: %.20e\n", h);
