...
static int test_wyman_cmf_sanity(void) {
    ...
    alwan_scalar ybar_555 = alwan_wyman_ybar_v(ALWAN_LITERAL(555.0));
    ...
    alwan_scalar zbar_440 = alwan_wyman_zbar_v(ALWAN_LITERAL(440.0));
    TEST_ASSERT(zbar_440 > ALWAN_LITERAL(1.0), "zbar(440) should be > 1.0");

    /* At 600 nm (peak of xbar), xbar should be large */
    alwan_scalar xbar_600 = alwan_wyman_xbar_v(ALWAN_LITERAL(600.0));
    TEST_ASSERT(xbar_600 > ALWAN_LITERAL(0.8), "xbar(600) should be > 0.8");

    /* Outside visible range, CMFs should be near zero */
    alwan_scalar xbar_380 = alwan_wyman_xbar_v(ALWAN_LITERAL(380.0));
    alwan_scalar ybar_380 = alwan_wyman_ybar_v(ALWAN_LITERAL(380.0));
...
static int test_hero_sample_range(void) {
    ...
    alwan_scalar lambda_0 = alwan_hero_wavelength_sample_v(ALWAN_LITERAL(0.0));
    TEST_ASSERT_NEAR(lambda_0, ALWAN_LITERAL(380.0), ALWAN_LITERAL(1e-10),
                     "hero sample u=0");

    /* u=1 should give 780 nm */
    alwan_scalar lambda_1 = alwan_hero_wavelength_sample_v(ALWAN_LITERAL(1.0));
    TEST_ASSERT_NEAR(lambda_1, ALWAN_LITERAL(780.0), ALWAN_LITERAL(1e-10),
                     "hero sample u=1");

    /* u=0.5 should give 580 nm */
    alwan_scalar lambda_mid = alwan_hero_wavelength_sample_v(ALWAN_LITERAL(0.5));
...
static int test_hero_pdf(void) {
    alwan_scalar pdf = alwan_hero_wavelength_pdf_v(ALWAN_LITERAL(550.0));
...
static int test_hero_stratified(void) {
    alwan_scalar hero = ALWAN_LITERAL(500.0);
    int N = 4;

    for (int i = 0; i < N; i++) {
        alwan_scalar lambda = alwan_hero_wavelength_stratified_v(hero, i, N);
    ...
    alwan_scalar first = alwan_hero_wavelength_stratified_v(hero, 0, N);
...
static int test_hero_to_xyz(void) {
    alwan_xyz xyz = alwan_hero_wavelength_to_xyz_v(ALWAN_LITERAL(555.0));
...
static int test_hero_api(void) {
    ...
    alwan_xyz xyz;
    status = alwan_hero_wavelength_to_xyz(&xyz, ALWAN_LITERAL(555.0));
    TEST_ASSERT(status == ALWAN_OK, "hero to_xyz api failed");
    TEST_ASSERT(xyz.y > ALWAN_LITERAL(0.8), "hero to_xyz api Y");

    /* Batch */
    alwan_scalar lambdas[4];
    alwan_xyz weights[4];
