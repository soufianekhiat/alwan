...
ALWAN_DIAG_POP
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        alwan_scalar diff_J = ALWAN_ABS(computed.J - expected.J);
        alwan_scalar diff_C = ALWAN_ABS(computed.C - expected.C);
        alwan_scalar diff_h = ALWAN_ABS(computed.h - expected.h);
