...
#include "reference_values/test_xyz_jzazbz_pairs.csv"
    ...
    for (size_t i = 0; i < num_colors; i++) {
...
#endif
        alwan_scalar diff_Jz = ALWAN_FABS(jzazbz_computed.Jz - jzazbz_expected.Jz);
        alwan_scalar diff_az = ALWAN_FABS(jzazbz_computed.az - jzazbz_expected.az);
        alwan_scalar diff_bz = ALWAN_FABS(jzazbz_computed.bz - jzazbz_expected.bz);
        ...
        alwan_scalar diff_x = ALWAN_FABS(xyz_out.x - xyz_in.x);
        alwan_scalar diff_y = ALWAN_FABS(xyz_out.y - xyz_in.y);
        alwan_scalar diff_z = ALWAN_FABS(xyz_out.z - xyz_in.z);
...
static int test_jzazbz_jzczhz_round_trip(void) {
    ...
    TEST_ASSERT(ALWAN_FABS(jzczhz.Jz - ALWAN_LITERAL(0.5)) < ALWAN_TEST_TOLERANCE, "Jz mismatch");
    TEST_ASSERT(ALWAN_FABS(jzczhz.Cz) < ALWAN_TEST_TOLERANCE, "Cz should be 0");

    alwan_jzczhz_to_jzazbz(&jzczhz, &jzazbz_out);
    TEST_ASSERT(ALWAN_FABS(jzazbz_out.Jz - jzazbz.Jz) < ALWAN_TEST_TOLERANCE,
                "Jzazbz round-trip failed (Jz)");
    TEST_ASSERT(ALWAN_FABS(jzazbz_out.az - jzazbz.az) < ALWAN_TEST_TOLERANCE,
                "Jzazbz round-trip failed (az)");
    TEST_ASSERT(ALWAN_FABS(jzazbz_out.bz - jzazbz.bz) < ALWAN_TEST_TOLERANCE,
    ...
    alwan_scalar diff_Jz = ALWAN_FABS(jzazbz_out.Jz - jzazbz.Jz);
    alwan_scalar diff_az = ALWAN_FABS(jzazbz_out.az - jzazbz.az);
    alwan_scalar diff_bz = ALWAN_FABS(jzazbz_out.bz - jzazbz.bz);
