 ...
 *
 * Test 310: P8.5 SPD Shape Descriptors
 *
...
static int test_shape_descriptor_d65(void) {
    ...
    status = alwan_spd_analyze_shape(&d65, &shape);
    ...
    TEST_ASSERT(ALWAN_FABS(shape.bandwidth - ALWAN_LITERAL(470.0)) < ALWAN_LITERAL(1.0),
...
static int test_shape_descriptor_led(void) {
    ...
    alwan_spd led;
    int status = alwan_spd_illuminant(ctx, ALWAN_ILLUMINANT_LED_B1, &led);
    TEST_ASSERT(status == ALWAN_OK, "Failed to load LED-B1 illuminant");

    /* Analyze shape */
    alwan_spd_shape shape;
    status = alwan_spd_analyze_shape(&led, &shape);
...
static int test_shape_descriptor_blackbody(void) {
    ...
    alwan_spd bb;
    int status = alwan_spd_blackbody(ctx, ALWAN_LITERAL(6500.0),
                                  ALWAN_LITERAL(360.0), ALWAN_LITERAL(830.0), 471, &bb);
    TEST_ASSERT(status == ALWAN_OK, "Failed to create blackbody");

    /* Analyze shape */
    alwan_spd_shape shape;
    status = alwan_spd_analyze_shape(&bb, &shape);
...
int test_310_spd_shape_main(void) {
    printf("\n========================================\n");
    printf("Test 310: SPD Shape Descriptors (P8.5)\n");
    printf("========================================\n\n");
