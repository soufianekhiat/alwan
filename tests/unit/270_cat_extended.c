 ...
 *
 * Test 270: P7 Extended Chromatic Adaptation Transforms (CAT)
 *
    ...
    alwan_scalar const *expected_adapted_data)
{
    ...
    int status = alwan_cat_matrix(&d65_xyz, &d50_xyz, method, &computed_matrix);
    ...
    for (size_t i = 0; i < num_test_colors * 3; i++) {
        alwan_scalar color_diff = ALWAN_FABS(adapted_xyz[i] - expected_adapted_data[i]);
...
int test_270_cat_extended_main(void) {
