 ...
 *
 * Test 320: P9 Gamut Analysis & Mapping
 *
...
static int test_spectral_locus_wavelengths(void) {
    ...
    for (size_t i = 0; i < 6; i++) {
        ...
        int status = alwan_spectral_locus_xy(wl, &xy_out);
...
static int test_spectral_locus_interpolation(void) {
    ...
    int status1 = alwan_spectral_locus_xy(ALWAN_LITERAL(500.0), &xy_500);
    int status2 = alwan_spectral_locus_xy(ALWAN_LITERAL(500.5), &xy_500_5);
    ...
    alwan_scalar diff_x = ALWAN_FABS(xy_500_5.v[0] - xy_500.v[0]);
    alwan_scalar diff_y = ALWAN_FABS(xy_500_5.v[1] - xy_500.v[1]);
...
static int test_dominant_wavelength_green(void) {
    ...
    int status = alwan_dominant_wavelength(&xy_green, &xy_d65,
                                            &wl_out, &xy_wl_out, &xy_cw_out);
...
static int test_excitation_purity_green(void) {
    ...
    int status = alwan_excitation_purity(&xy_green, &xy_d65, &purity_out);
...
static int test_dominant_wavelength_red(void) {
    ...
    int status = alwan_dominant_wavelength(&xy_red, &xy_d65, &wl_out, NULL, NULL);
    TEST_ASSERT(status == ALWAN_OK, "Failed to compute dominant wavelength");
...
static int test_dominant_wavelength_blue(void) {
    ...
    int status = alwan_dominant_wavelength(&xy_blue, &xy_d65, &wl_out, NULL, NULL);
    TEST_ASSERT(status == ALWAN_OK, "Failed to compute dominant wavelength");
...
static int test_gamut_volume_ratio(void) {
    ...
    int status1 = alwan_gamut_volume_ratio(&bt2020, &srgb, &ratio_bt2020_srgb);
    int status2 = alwan_gamut_volume_ratio(&srgb, &bt2020, &ratio_srgb_bt2020);
    ...
    TEST_ASSERT(ALWAN_FABS(product - ALWAN_LITERAL(1.0)) < ALWAN_LITERAL(0.01),
                "Ratios should be reciprocals");
...
static int test_gamut_coverage(void) {
    ...
    int status1 = alwan_gamut_coverage(&srgb, &bt2020, 50000, 12345, &coverage_srgb_by_bt2020);
    ...
    int status2 = alwan_gamut_coverage(&bt2020, &srgb, 50000, 12345, &coverage_bt2020_by_srgb);
...
int test_320_gamut_analysis_main(void) {
    printf("\n========================================\n");
    printf("Test 320: P9 Gamut Analysis & Mapping\n");
    printf("========================================\n\n");
