...
#include "data/d65_xy.csv"
    ...
    alwan_scalar diff_x = ALWAN_FABS(g_d65_xy[0] - expected_d65[0]);
    alwan_scalar diff_y = ALWAN_FABS(g_d65_xy[1] - expected_d65[1]);
...
static int test_srgb_primaries(void) {
    ...
    for (int i = 0; i < 6; i++) {
        alwan_scalar diff = ALWAN_FABS(g_srgb_primaries[i] - expected[i]);
        if (diff > 1e-5) {
