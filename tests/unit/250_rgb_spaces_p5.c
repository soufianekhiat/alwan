...
static int test_matrix_derivation(char const *name, alwan_rgb_space_desc const *desc) {
    ...
    alwan_mat3_mul(&rgb_to_xyz, &xyz_to_rgb, &identity);

    /* Check diagonal is ~1 */
    alwan_scalar diag_err = 0.0;
    diag_err += ALWAN_FABS(identity.m[0] - ALWAN_LITERAL(1.0));
    diag_err += ALWAN_FABS(identity.m[4] - ALWAN_LITERAL(1.0));
    diag_err += ALWAN_FABS(identity.m[8] - ALWAN_LITERAL(1.0));

    /* Check off-diagonal is ~0 */
    alwan_scalar offdiag_err = 0.0;
    offdiag_err += ALWAN_FABS(identity.m[1]);
    offdiag_err += ALWAN_FABS(identity.m[2]);
    offdiag_err += ALWAN_FABS(identity.m[3]);
    offdiag_err += ALWAN_FABS(identity.m[5]);
    offdiag_err += ALWAN_FABS(identity.m[6]);
    offdiag_err += ALWAN_FABS(identity.m[7]);
...
static int test_adobe_rgb_1998(void) {
    ...
    int status = alwan_rgb_get_space_descriptor(ctx, ALWAN_RGB_SPACE_ADOBE_RGB_1998, &desc);
    ...
    alwan_scalar white_err = ALWAN_FABS(desc.white_xy[0] - d65_x) + ALWAN_FABS(desc.white_xy[1] - d65_y);
    TEST_ASSERT(white_err < ALWAN_LITERAL(0.001),
                "Adobe RGB 1998 should have D65 white point");
...
static int test_prophoto_rgb(void) {
    ...
    alwan_rgb_space_desc desc;
    int status = alwan_rgb_get_space_descriptor(ctx, ALWAN_RGB_SPACE_PROPHOTO_RGB, &desc);
    ...
    alwan_scalar white_err = ALWAN_FABS(desc.white_xy[0] - d50_x) + ALWAN_FABS(desc.white_xy[1] - d50_y);
    TEST_ASSERT(white_err < ALWAN_LITERAL(0.001),
                "ProPhoto RGB should have D50 white point");
...
static int test_cinema_spaces(void) {
    ...
    for (size_t i = 0; i < sizeof(cinema_spaces) / sizeof(cinema_spaces[0]); i++) {
        alwan_rgb_space_desc desc;
        int status = alwan_rgb_get_space_descriptor(ctx, cinema_spaces[i].space, &desc);
...
static int test_legacy_spaces(void) {
    ...
    for (size_t i = 0; i < sizeof(legacy_spaces) / sizeof(legacy_spaces[0]); i++) {
        alwan_rgb_space_desc desc;
        int status = alwan_rgb_get_space_descriptor(ctx, legacy_spaces[i].space, &desc);
...
static int test_core_spaces(void) {
    ...
    for (size_t i = 0; i < sizeof(core_spaces) / sizeof(core_spaces[0]); i++) {
        alwan_rgb_space_desc desc;
        int status = alwan_rgb_get_space_descriptor(ctx, core_spaces[i].space, &desc);
...
static int test_rgb_space_conversion(void) {
    ...
    int status = alwan_rgb_get_space_descriptor(ctx, ALWAN_RGB_SPACE_SRGB, &srgb_desc);
    TEST_ASSERT(status == ALWAN_OK, "Failed to get sRGB descriptor");
    status = alwan_rgb_get_space_descriptor(ctx, ALWAN_RGB_SPACE_ADOBE_RGB_1998, &adobe_desc);
    TEST_ASSERT(status == ALWAN_OK, "Failed to get Adobe RGB 1998 descriptor");
...
static int test_invalid_space_name(void) {
    ...
    int status = alwan_rgb_get_space_descriptor(ctx, (alwan_rgb_space)9999, &desc);
...
int test_250_rgb_spaces_p5_main(void) {
