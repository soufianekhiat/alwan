...
static int test_munsell_neutrals(void) {
    ...
    /* N0 (black) */
    status = alwan_munsell_to_xyz(0.0, 0.0, 0.0, ALWAN_ILLUMINANT_C, &xyz);
    if (status != ALWAN_OK) {
...
static int test_munsell_chromatic(void) {
    ...
    status = alwan_munsell_to_xyz(5.0, 5.0, 10.0, ALWAN_ILLUMINANT_C, &xyz);
    if (status != ALWAN_OK) {
...
static int test_munsell_roundtrip(void) {
    ...
    status = alwan_xyz_to_munsell(&xyz, ALWAN_ILLUMINANT_C, &hue_out, &value_out, &chroma_out);
    if (status != ALWAN_OK) {
...
static int test_munsell_illuminant_adaptation(void) {
    ...
    status = alwan_munsell_to_xyz(0.0, 5.0, 0.0, ALWAN_ILLUMINANT_C, &xyz_c);
    if (status != ALWAN_OK) {
        TEST_FAIL("Failed to convert under C: error %d", status);
    }

    /* Convert N5 under Illuminant D65 */
    status = alwan_munsell_to_xyz(0.0, 5.0, 0.0, ALWAN_ILLUMINANT_D65, &xyz_d65);
    if (status != ALWAN_OK) {
...
static int test_colorchecker_classic_patches(void) {
    ...
    status = alwan_color_checker_data(ALWAN_COLORCHECKER_CLASSIC, ALWAN_ILLUMINANT_D50,
                                       18, &xyz);  /* Index 18 = patch 19 */
    if (status != ALWAN_OK) {
...
static int test_colorchecker_illuminant_adaptation(void) {
    ...
    status = alwan_color_checker_data(ALWAN_COLORCHECKER_CLASSIC, ALWAN_ILLUMINANT_D65,
                                       0, &xyz_d65);
    if (status != ALWAN_OK) {
...
static int test_colorchecker_bounds(void) {
    ...
    status = alwan_color_checker_data(ALWAN_COLORCHECKER_CLASSIC, ALWAN_ILLUMINANT_D50,
                                       24, &xyz);  /* Index 24 is out of bounds (0-23) */
    if (status == ALWAN_OK) {
        TEST_FAIL("Expected error for out-of-bounds index");
    }

    /* Test invalid type */
    status = alwan_color_checker_data((alwan_colorchecker_type)999, ALWAN_ILLUMINANT_D50,
                                       0, &xyz);
    if (status == ALWAN_OK) {
...
static int test_ncs_parsing(void) {
    ...
    status = alwan_ncs_to_xyz("S 1050-Y90R", &xyz);
...
int test_350_reference_data_main(void) {
    printf("========================================\n");
