...
#define ROUND_TRIP_TOLERANCE ALWAN_LITERAL(1e-5)
#define KNOWN_VALUE_TOLERANCE ALWAN_LITERAL(1e-4)
#else
#define ROUND_TRIP_TOLERANCE ALWAN_LITERAL(1e-10)
#define KNOWN_VALUE_TOLERANCE ALWAN_LITERAL(1e-6)
#endif
...
static int test_blue_light_fix_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        TEST_ASSERT(diff_r < ROUND_TRIP_TOLERANCE && diff_g < ROUND_TRIP_TOLERANCE && diff_b < ROUND_TRIP_TOLERANCE, "Round-trip exceeded");
    }
...
static int test_acescc_known_values(void) {
    ...
    for (size_t i = 0; i < num_pairs; i++) {
        ...
        TEST_ASSERT(diff < KNOWN_VALUE_TOLERANCE, "ACEScc known value exceeded");
    }
...
static int test_acescc_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_values; i++) {
        ...
        TEST_ASSERT(rel < ROUND_TRIP_TOLERANCE, "ACEScc round-trip exceeded");
    }
...
static int test_acescct_known_values(void) {
    ...
    for (size_t i = 0; i < num_pairs; i++) {
        ...
        TEST_ASSERT(ALWAN_ABS(encoded - known_pairs[i].encoded) < KNOWN_VALUE_TOLERANCE, "ACEScct known value exceeded");
    }
...
static int test_acescct_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_values; i++) {
        ...
        TEST_ASSERT(ALWAN_ABS(decoded - test_values[i]) < ROUND_TRIP_TOLERANCE, "ACEScct round-trip exceeded");
    }
...
static int test_tf_roundtrip(char const *name, alwan_transfer_function tf) {
    ...
    for (size_t i = 0; i < num_values; i++) {
        ...
        if (ALWAN_ABS(decoded - test_values[i]) > ROUND_TRIP_TOLERANCE) return 1;
    }
...
static int test_adx10_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_values; i++) {
        ...
        TEST_ASSERT(ALWAN_ABS(decoded - test_values[i]) < ROUND_TRIP_TOLERANCE, "ADX10 round-trip exceeded");
    }
...
static int test_adx16_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_values; i++) {
        ...
        TEST_ASSERT(ALWAN_ABS(decoded - test_values[i]) < ROUND_TRIP_TOLERANCE, "ADX16 round-trip exceeded");
    }
...
static int test_adx_known_values(void) {
    ...
    TEST_ASSERT(ALWAN_ABS(encoded - expected) < KNOWN_VALUE_TOLERANCE, "ADX10 known value exceeded");
...
static int test_dlog_known_values(void) {
    ...
    for (size_t i = 0; i < num_pairs; i++) {
        ...
        TEST_ASSERT(diff < KNOWN_VALUE_TOLERANCE, "D-Log known value exceeded");
    }
...
/* Tolerance for inverse functions (RedMod, Glow)
 *
 * Implementation uses OCIO analytical formula as initial estimate, then
 * iterative refinement (3 Newton iterations) to achieve machine precision.
 * This gives exact round-trip within floating-point precision limits.
 *
 * Tolerances are set to standard floating-point precision:
 * - double: 1e-6 (analytical precision)
 * - float: 1e-4 (single-precision limits) */
#if ALWAN_SCALAR_IS_FLOAT
#define APPROX_INV_TOLERANCE ALWAN_LITERAL(1e-4)  /* float precision */
#define COMPOUND_INV_TOLERANCE ALWAN_LITERAL(1e-4)  /* float precision */
#else
#define APPROX_INV_TOLERANCE ALWAN_LITERAL(1e-6)  /* double precision */
#define COMPOUND_INV_TOLERANCE ALWAN_LITERAL(1e-6)  /* double precision */
#endif
...
static int test_aces_look_1_0_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        alwan_scalar diff_g = ALWAN_ABS(roundtrip.g - test_colors[i].g);
        alwan_scalar diff_b = ALWAN_ABS(roundtrip.b - test_colors[i].b);
        if (diff_r >= COMPOUND_INV_TOLERANCE || diff_g >= COMPOUND_INV_TOLERANCE || diff_b >= COMPOUND_INV_TOLERANCE) {
        ...
        TEST_ASSERT(diff_r < COMPOUND_INV_TOLERANCE && diff_g < COMPOUND_INV_TOLERANCE && diff_b < COMPOUND_INV_TOLERANCE, "ACES 1.0 Look round-trip exceeded");
    }
...
static int test_glow_inv_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        TEST_ASSERT(diff < ROUND_TRIP_TOLERANCE * 3, "Glow10 round-trip exceeded");
        /* Test Glow03 */
        ...
        diff = ALWAN_ABS(roundtrip.r - test_colors[i].r) +
               ALWAN_ABS(roundtrip.g - test_colors[i].g) +
               ALWAN_ABS(roundtrip.b - test_colors[i].b);
        TEST_ASSERT(diff < ROUND_TRIP_TOLERANCE * 3, "Glow03 round-trip exceeded");
    }
...
static int test_redmod_inv_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        TEST_ASSERT(diff < APPROX_INV_TOLERANCE, "RedMod03 round-trip exceeded");
    }
...
static int test_ap0_ap1_roundtrip(void) {
    ...
    TEST_ASSERT(max_diff < KNOWN_VALUE_TOLERANCE, "Neutral axis not preserved in AP0->AP1");
...
static int test_jmh_roundtrip(void) {
    ...
    for (size_t i = 0; i < num_colors; i++) {
        ...
        alwan_scalar diff = ALWAN_ABS(roundtrip.r - test_colors[i].r) +
                            ALWAN_ABS(roundtrip.g - test_colors[i].g) +
                            ALWAN_ABS(roundtrip.b - test_colors[i].b);
        TEST_ASSERT(diff < KNOWN_VALUE_TOLERANCE * 3, "JMh round-trip exceeded");
    }
...
static int test_parametric_lmt(void) {
    ...
    TEST_ASSERT(ALWAN_ABS(result.r - gray.r) < ROUND_TRIP_TOLERANCE, "Identity should preserve R");
    TEST_ASSERT(ALWAN_ABS(result.g - gray.g) < ROUND_TRIP_TOLERANCE, "Identity should preserve G");
    TEST_ASSERT(ALWAN_ABS(result.b - gray.b) < ROUND_TRIP_TOLERANCE, "Identity should preserve B");
    ...
    TEST_ASSERT(ALWAN_ABS(result.r - ALWAN_LITERAL(0.36)) < ROUND_TRIP_TOLERANCE, "Slope should double values");
    ...
    TEST_ASSERT(ALWAN_ABS(result.r - ALWAN_LITERAL(0.28)) < ROUND_TRIP_TOLERANCE, "Offset should add 0.1");
    alwan_aces_lmt_params_init(&params);  /* Reset */
    ...
    TEST_ASSERT(ALWAN_ABS(result.r - result.g) < ROUND_TRIP_TOLERANCE, "Gray should stay neutral");
    TEST_ASSERT(ALWAN_ABS(result.g - result.b) < ROUND_TRIP_TOLERANCE, "Gray should stay neutral");
    alwan_aces_lmt_params_init(&params);  /* Reset */
...
static int test_tonescale_known_values(void) {
    ...
    TEST_ASSERT(ALWAN_ABS(result.r - result.g) < KNOWN_VALUE_TOLERANCE, "Neutral should stay neutral");
    TEST_ASSERT(ALWAN_ABS(result.g - result.b) < KNOWN_VALUE_TOLERANCE, "Neutral should stay neutral");
...
static int test_gamut_compress_boundary(void) {
    ...
    TEST_ASSERT(jmh_compressed.v[1] <= jmh.v[1] + KNOWN_VALUE_TOLERANCE, "Gamut compress should reduce or preserve M");
    ...
    TEST_ASSERT(diff < APPROX_INV_TOLERANCE * 3, "Gamut compress round-trip exceeded");
