...
Choose `float` (7 digits, faster) or `double` (16 digits, default) at compile time. Test tolerances automatically adapt to your chosen precision. Favorize `double` to test against colour-science expected results.
...
```powershell
.\generate_data_new.ps1       # Generate data files (illuminants, CMFs, matrices)
.\generate_data_tests_new.ps1 # Generate test reference values
```
