...
- Validated against Python's [Colour](https://github.com/colour-science/colour) library
...
2. **Build and test (Sharpmake/MSVC):**
   ```batch
...
Runtime data loading (`ALWAN_EMBED_DATA=0`) is not implemented; only embedded mode is currently supported.
...
**Data Generation:**
Reference data and test fixtures are generated by Python scripts in `../alwan_gendata/` via the included orchestrator:
```powershell
./generate_data.ps1       # regenerate embedded data (CMFs, illuminants, matrices)
./generate_data_tests.ps1 # regenerate test reference values
```
...
│   └── map/               # SIMD-accelerated bulk operations
├── tests/                  # Self-contained test suite (84 suites)
...
│   └── tests/             # Test reference generators
├── buildsystem/            # Sharpmake build scripts
│   └── sharpmake/         # .cs project definitions
├── docs/                   # Documentation
...
- ✅ Unified test suite (84 test suites)
