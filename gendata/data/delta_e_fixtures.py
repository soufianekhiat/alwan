"""
Generate Delta-E metric test fixtures.
Source: colour.delta_E functions

Test Lab pairs are hardcoded (inputs).
Expected Delta-E values are computed from colour-science.
"""
...
# Delta-E test pairs (inputs - hardcoded as per requirements)
DELTA_E_TEST_PAIRS = [
...
def generate_delta_e_fixtures(output_dir):
    ...
    print("\nGenerating Delta-E metric test fixtures...")
    ...
    # Compute and save Delta-E values for each metric
    metrics = {
        'delta_e_76': lambda l1, l2: colour.difference.delta_E_CIE1976(l1, l2),
        'delta_e_94': lambda l1, l2: colour.difference.delta_E_CIE1994(l1, l2),
        'delta_e_cmc_2_1': lambda l1, l2: colour.difference.delta_E_CMC(l1, l2, l=2, c=1),
        'delta_e_00': lambda l1, l2: colour.difference.delta_E_CIE2000(l1, l2),
    }
    ...
    for filename, func in metrics.items():
        ...
        save_vector(delta_e_values, filepath, f"Delta-E values for {filename}")
